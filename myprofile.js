...
function booksSheet(){var sel=myBooks().slice();var sh=openSheet('<h3>My Books</h3><div class="rp-sub">Tap the platforms you use. Picks highlight your books first.</div><div id="rpPills"></div><button class="rp-btn" id="rpSaveBooks">Save</button>'+(sel.length?'<button class="rp-btn ghost" id="rpClearBooks">Clear my books</button>':''));
...
sh.querySelector('#rpSaveBooks').onclick=function(){sv('rp_books',sel);sv('rp_books_asked',1);personalize();closeModal();toast(sel.length?'Books saved':'Saved');};
...
function betsSheet(){var h='<h3>My RixPicks</h3><div class="rp-tabs"><div class="rp-tab on" id="tabBets">My Bets</div><div class="rp-tab" id="tabBooks">My Books</div></div><div id="rpBody"></div>';
var sh=openSheet(h);
...
var h='<div class="rp-sub">Your books: '+(mine.length?mine.map(bookName).join(', '):'none yet')+'</div><button class="rp-btn" id="rpEditBooks">Edit my books</button>';
